...
model = LogisticRegressionModel(784, 10, 0.01)

# Train the model
train_loss, train_accuracy, val_loss, val_accuracy = \
    model.train(normalize(scale(X_train)), y_train, normalize(scale(X_val)), y_val, 0.001, 100, 50)
...
np.savetxt("LR_pred.csv", y_test, delimiter="\n", fmt="%d")
